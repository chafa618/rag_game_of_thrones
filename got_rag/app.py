...
_, chunk_id_mapping = load_data('../data/jdt_chunks_sentences_256.json')
index = load_index('index_juego_de_tronos_chunk_256.ann', 768)
...
async def openai_endpoint(request: QueryRequest):
    try:
        answer = get_answer_from_openai(request.query, request.context)
...
async def ollama_endpoint(request: QueryRequest):
    ...
    try:
        candidates = get_rag_candidates(model, request.query, index, chunk_id_mapping)
